...
public class ComputerDAO {
	...
	private static final String HQL_UPDATE = "UPDATE ComputerDTOPersistance SET name=:name, introduced=:introduced, discontinued=:discontinued, companyDTOPersistance.id=:companyId WHERE id=:id";
	private static final String HQL_DELETE = "DELETE FROM ComputerDTOPersistance WHERE id=:id";
	private static final String HQL_SELECT = "FROM ComputerDTOPersistance computer left join fetch computer.companyDTOPersistance as company WHERE computer.id = :id";
	private static final String HQL_ALL_COMPUTER = "FROM ComputerDTOPersistance ORDER BY id";
	private static final String HQL_ALL_COMPUTER_PAGINATION = "FROM ComputerDTOPersistance as computer ORDER BY ORDERATTRIBUTE ORDERSORT  ";
	private static final String HQL_COUNT_ALL_COMPUTER = "SELECT COUNT(id) FROM ComputerDTOPersistance ";
//	private static final String SQL_SEARCH_BY_NAME_COMPA_COMPU = "SELECT New com.excilys.cdb.dto.ComputerDTOPersistance(computer.id, computer.name, computer.introduced, computer.discontinued, computer.companyDTOPersistance)  FROM ComputerDTOPersistance computer left join computer.companyDTOPersistance as company WHERE lower(computer.name) LIKE :nameComputer OR lower(company.name) LIKE :nameCompany ORDER BY ORDERATTRIBUTE ORDERSORT ";
	private static final String HQL_SEARCH_BY_NAME_COMPA_COMPU = "FROM ComputerDTOPersistance computer left join fetch computer.companyDTOPersistance as company WHERE lower(computer.name) LIKE :nameComputer OR lower(company.name) LIKE :nameCompany ORDER BY ORDERATTRIBUTE ORDERSORT ";
	private static final String HQL_SEARCH_BY_NAME_COUNT = "SELECT COUNT(computer.id) FROM ComputerDTOPersistance computer left join computer.companyDTOPersistance  company WHERE lower(computer.name) LIKE :nameComputer OR lower(company.name) LIKE :nameCompany ";
